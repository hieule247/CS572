...
module.exports.repairsAddOne = function(req, res){
    ...
    var recvData = {
        ...
        pricePredict: req.body.pricePredict, 
        price: req.body.price, 
        status: req.body.status, 
        notes: req.body.notes        
    }
    console.log("SERVER....Receive data....:", recvData);
    // 3. MAIN
    Repair.create({
        pid: req.body.pid, 
        machineType: req.body.machineType, 
        ...
        function(err, doc){
            ...
            // 3.9: Send back to client
            console.log("new item values...:", response.message);
            res.status(response.status).json(response.message);  
        }
    );
}
...
// ---- Search By Name: checking and hardening 
module.exports.repairsSearch = function(req, res) {
    var nameFunc = "repairsSearch";
    // 0. log for debuging
    ...
    // using Mongoose
    Repair.find({pid: pid}).exec(function(err, repairs) {
        if (err){
            response.status = 500; // server error
            response.message= err;
        } else {
            response.status = 200; // ok
            response.message= repairs;
        }
